...
function setDomainUrl(source) {
    if (source == "success") {
        var bg = chrome.extension.getBackgroundPage();
        bg.setTargetUrl(strUrl);
        alert("锁定成功");
    } else {
...
$('#SetUrl').click(() => {
    strUrl = $('#UrlValue').val();
    var strRegex = /([\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?)/;
    var re = new RegExp(strRegex);

    if (!re.test(strUrl)) {
        alert("校验失败,请检查格式是否正确");
    } else {
        setDomainUrl('success')
    }
});
...
$('#getTracking_Number').click(() => {
    sendMessageToContentScript('get_tracking_number', function (response) {
        if (response) {
